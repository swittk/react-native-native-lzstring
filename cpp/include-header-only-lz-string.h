 ...
 * Modified 2022 Switt Kongdachalert
 *
 */
...
// inspired by code from qt-lzstring
string compressToUTF16(const string& input) {
    if(input.empty()) return {};
    using namespace __inner;
    string result = _compress(input, 15, [&](int index) { return index + 32; });
    result.append(u" ");
    return result;
}
// inspired by code from qt-lzstring
string decompressFromUTF16(const string& input) {
    if(input.empty()) return {};
    using namespace __inner;
    const char16_t *m_compressed = input.c_str();
    return _decompress((int)input.length(), 16384, [&](int index) {
        return m_compressed[index] - 32;
    });
}
// inspired by code from qt-lzstring
string compress(const string& input) {
    if(input.empty()) return {};
    using namespace __inner;
    return _compress(input, 16, [&](int index) {
        return index;
    });
}
// inspired by code from qt-lzstring
string decompress(const string& input) {
    ...
    using namespace __inner;
    const char16_t *m_compressed = input.c_str();
    return _decompress((int)input.length(), 32768, [&](int index) {
        return m_compressed[index];
    });
}
// clang-format on
